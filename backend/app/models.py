...
class Holding(Base):
    ...
    variacion_dia_prev: Mapped[float | None] = mapped_column(Numeric(10, 4), nullable=True)
...
class CryptoSale(Base):
    """Venta realizada de una tenencia crypto. Registra el P&L realizado
    calculado contra el costo promedio (`costo_usd_unit`) de la tenencia al
    momento de la venta. Es histórico — la tenencia se reduce por separado."""

    __tablename__ = "crypto_sales"
    ...
    cantidad: Mapped[float] = mapped_column(Numeric(28, 12), nullable=False, default=0)
    costo_usd_unit: Mapped[float | None] = mapped_column(Numeric(18, 8), nullable=True)
    price_usd: Mapped[float] = mapped_column(Numeric(18, 8), nullable=False, default=0)
    proceeds_usd: Mapped[float] = mapped_column(Numeric(18, 2), nullable=False, default=0)
    cost_total_usd: Mapped[float | None] = mapped_column(Numeric(18, 2), nullable=True)
    pnl_usd: Mapped[float | None] = mapped_column(Numeric(18, 2), nullable=True)
    pnl_pct: Mapped[float | None] = mapped_column(Numeric(18, 4), nullable=True)
    dolar_rate: Mapped[float] = mapped_column(Numeric(18, 4), nullable=False, default=0)
    notas: Mapped[str | None] = mapped_column(String(500), nullable=True)
